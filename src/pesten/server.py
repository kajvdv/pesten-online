...
async def connect_to_lobby(websocket: WebSocket, name):
    ...
    on_turn = asyncio.Event()
    flags.append(on_turn)
    # Telling the first player to make a move
    if len(sockets) == 2:
        flags[0].set()
    #TODO: Send the initial board to the player
    while(True):
        # pauzeer totdat het de beurt krijgt
        await on_turn.wait()
        ...
        # get next index. Only after game says it was correct.
        next_index = int(game_process.stdout.readline().strip())
        current_index = sockets.index(websocket)
        if next_index != current_index:
            on_turn.clear()
            flags[next_index].set()
